...
def _run(tool: Tool, stage: Path, command_id: str, args: tuple[str,...], bam: Path, index: Path, *, output: Path|None=None, overwrite=False):
    ...
    spec=CommandSpec(command_id,"01_prepare_bam",tool.tool_id.value,(str(tool.resolved_executable),*args),str(stage.resolve()),
        declared_inputs=(InputDeclaration("bam",str(bam)),InputDeclaration("index",str(index))),declared_outputs=declared,overwrite=overwrite)
    return execute(spec,tool,record,stage/"logs")


def _link_or_copy(source: Path, destination: Path) -> str:
    try: os.link(source,destination); return "HARD_LINK"
...
def prepare_bam(source_bam: str|Path, source_index: str|Path, output_directory: str|Path, tool: Tool, *, overwrite=False) -> dict[str,Any]:
    ...
    _run(tool,stage,"quickcheck-source",("quickcheck",str(source_bam)),source_bam,source_index)
    header_record=_run(tool,stage,"header-source",("view","-H",str(source_bam)),source_bam,source_index)
    flag_record=_run(tool,stage,"flagstat-source",("flagstat",str(source_bam)),source_bam,source_index)
    idx_record=_run(tool,stage,"idxstats-source",("idxstats",str(source_bam)),source_bam,source_index)
    ...
    if overwrite:
        prepared.unlink(missing_ok=True); prepared_index.unlink(missing_ok=True)
    if sort_order=="coordinate":
        ...
        _run(tool,stage,"sort",("sort","-o",str(temporary),str(source_bam)),source_bam,source_index,output=temporary,overwrite=True)
        _run(tool,stage,"quickcheck-sorted",("quickcheck",str(temporary)),temporary,source_index)
        os.replace(temporary,prepared)
        _run(tool,stage,"index-prepared",("index","-o",str(prepared_index),str(prepared)),prepared,source_index,output=prepared_index,overwrite=overwrite)
        strategy="SORTED"; index_strategy="GENERATED"
    # Prove the prepared index can service the prepared BAM.
    _run(tool,stage,"quickcheck-prepared",("quickcheck",str(prepared)),prepared,prepared_index)
    ...
    metadata={"schema_version":"1.0","source_bam_sha256":sha256_file(source_bam),"source_index_sha256":sha256_file(source_index),
        ...
        "command_record_paths":[header_record["stdout_log"],flag_record["stdout_log"],idx_record["stdout_log"]]}
