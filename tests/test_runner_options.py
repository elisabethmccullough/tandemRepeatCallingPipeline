...
def test_stage_registry_has_workflow_roles_and_tools():
    ...
    assert stages["08_normalize_outputs"].required_input_roles == (
        "PACKAGE_PATIENT_FASTA", "PACKAGE_PATIENT_METADATA", "LOCUS_CONFIG")
    assert "STRAGLR_NORMALIZED_EVIDENCE" in stages["08_normalize_outputs"].optional_input_roles
...
def test_stage08_runner_dry_run_writes_plan_not_evidence(tmp_path):
    config, root, _, _ = make_run(tmp_path)
    run(config, dry_run=True, start_stage="08_normalize_outputs", stop_stage="08_normalize_outputs")
    record=json.loads((root/"00_manifest/stages/08_normalize_outputs.json").read_text())
    plan=json.loads((root/"09_normalized_evidence_plans/dry-run-validation-report.json").read_text())
    assert record["status"] == "DRY_RUN"
    assert plan["normalization_performed"] is False
    assert len([group for group in plan["caller_artifact_groups"] if group["status"] == "ABSENT"]) == 4
